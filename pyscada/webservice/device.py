...
class Device(GenericDevice):
    ...
    def __init__(self, device):
        ...
        for var in self.device.variable_set.filter(active=1):
            ...
            for ws in var.ws_variables.filter(active=1, webservice_RW=0):
                ...
                self.webservices[ws.pk]["variables"][var.pk][
                    "device_path"
                ] = var.device.webservicedevice.url
                self.webservices[ws.pk]["variables"][var.pk][
                    "proxy"
                ] = var.device.webservicedevice.http_proxy
                self.webservices[ws.pk]["variables"][var.pk][
                    "variable_path"
                ] = var.webservicevariable.path
            ...
            for vp in var.variableproperty_set.all():
                ...
                for ws in vp.ws_variable_properties.filter(active=1, webservice_RW=0):
                    ...
                    self.webservices[ws.pk]["variable_properties"][vp.pk][
                        "device_path"
                    ] = vp.variable.device.webservicedevice.url
                    self.webservices[ws.pk]["variable_properties"][vp.pk][
                        "proxy"
                    ] = vp.variable.device.webservicedevice.http_proxy
                    self.webservices[ws.pk]["variable_properties"][vp.pk][
                        "variable_path"
                    ] = vp.variable.webservicevariable.path
    ...
    def write_data(self, variable_id, value, task):
        ...
        if value is not None and self.variables[variable_id]["object"].update_value(
            value, time()
        ):
            output.append(
                self.variables[variable_id]["object"].create_recorded_data_element()
            )
    ...
    def loop(self):
        ...
        if len(self.ws_write_todo) > 0:
            for ws_id in self.ws_write_todo:
                ...
                cwt = DeviceWriteTask(
                    variable_id=WebServiceAction.objects.get(id=ws_id).write_trigger.pk,
                    value=0,
                    start=time(),
                    user=DeviceWriteTask.objects.filter(
                        done=True,
                        variable=WebServiceAction.objects.get(id=ws_id).write_trigger,
                    )
                    .latest("start")
                    .user,
                )
                ...
                if len(tmp_data) > 0:
                    if hasattr(task.variable, "webservicevariable") and task.value:
                        for ws in task.variable.ws_write_trigger.filter(
                            active=1, webservice_RW=1, write_trigger=task.variable
                        ):
