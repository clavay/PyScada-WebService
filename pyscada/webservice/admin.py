...
class WebServiceDeviceAdmin(DeviceAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        return super(WebServiceDeviceAdmin, self).formfield_for_foreignkey(
    ...
    inlines = [WebServiceDeviceAdminInline]
...
class WebServiceVariableAdmin(VariableAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        return super(WebServiceVariableAdmin, self).formfield_for_foreignkey(
            db_field, request, **kwargs
        )
    ...
    inlines = [WebServiceVariableAdminInline]
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == "variables":
            kwargs["queryset"] = Variable.objects.filter(
                device__protocol_id=PROTOCOL_ID
            )
        elif db_field.name == "variable_properties":
            kwargs["queryset"] = VariableProperty.objects.filter(
                variable__device__protocol_id=PROTOCOL_ID
            )
